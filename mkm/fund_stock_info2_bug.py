...
def getstockinfo(fundcode,fundname):
    try:
        ...
        driver.get(url)
        time.sleep(5)
        # wait.until(lambda d: driver.find_elements_by_xpath('//div[@class="poptableWrap"]/table[@class="ui-table-hover"]/tbody/tr'))
...
if __name__ == "__main__":
    try:
        ...
        fundCountSql = "select count(1) from gp.fund_main_info where length(fundname) >0"
        count = db.findOne(conn,fundCountSql)
        indexPage=179
        indexData=0
        page=10
        if count != None:
            while indexData < count[0]:
                ##下一页
                indexData = indexPage * page + 1
                fundSql = "select fundcode,fundname from gp.fund_main_info where length(fundname) >0  order by fundcode desc limit {indexData},{page}"
                fundSql = fundSql.format(indexData=indexData,page=page)
                ...
                print "end:"+"index:"+str(indexPage)+"----indexData"+str(indexData)
                time.sleep(300)
