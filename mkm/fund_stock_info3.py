...
def getstockinfo(fundcode,fundname):
    try:
        ...
        for row in datas:
            ...
            insertFundSql = insertFundSql.format(fundcode=row["fundcode"], fundname=row["fundname"],
                                                 gpnum=str(row["gpnum"]), gpcode=str(row["gpcode"]),
                                                 gpname=row["gpname"], gprate=str(row["gprate"]))
            print str(row["fundcode"]) + "---" + row["fundname"] + "-----" + str(row["gpnum"]) + "-----" + str(
                row["gpcode"]) + row["gpname"] + "-----" + str(row["gprate"])
            print insertFundSql
...
if __name__ == "__main__":
    ...
    for fc in fundcodes:
        if index > 0:
            ...
            ###获取股票信息
            pool.apply_async(getstockinfo,(fc[0],fc[1]))
        if index % 10 == 0:
            time.sleep(120)
        index=index+1
    pool.close()
